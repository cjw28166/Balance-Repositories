        ...
        if (Gimbal.FireMode==GM_FIRE_READY) 
				{						
						if((SHOOT_COMMAND_KEYMAP)//收到操作手发弹指令
							&&(((Gimbal.ControlMode==GM_AIMBOT_OPERATE||Gimbal.ControlMode==GM_AIMBOT_RUNES)&&(((int)(Aimbot.State)&0x02)!=0))||Gimbal.ControlMode==GM_MANUAL_OPERATE||auto_fire_flag==0)//且视觉开火允许/无自动开火
